...
def average_normals(average_type: Union[str, int] = 'CUSTOM_NORMAL',
                    weight: int = 50,
                    threshold: float = 0.01):
    """Average custom normals of selected vertices 

    :param average_type: Type, Averaging methodCUSTOM_NORMAL Custom Normal,
    Take Average of vert Normals.FACE_AREA Face Area, Set all vert normals by
    Face Area.CORNER_ANGLE Corner Angle, Set all vert normals by Corner Angle.
    :type average_type: Union[str, int]
    :param weight: Weight, Weight applied per face 
    :type weight: int
    :param threshold: Threshold, Threshold value for different weights to be
    considered equal
...
def bevel(offset_type: Union[str, int] = 'OFFSET',
          offset: float = 0.0,
          ...
          face_strength_mode: Union[str, int] = 'NONE',
          miter_outer: Union[str, int] = 'SHARP',
          miter_inner: Union[str, int] = 'SHARP',
          spread: float = 0.1,
          use_custom_profile: bool = False,
          vmesh_method: Union[str, int] = 'ADJ',
          release_confirm: bool = False):
    """Cut into selected items at an angle to create bevel or chamfer 

    :param offset_type: Width Type, What distance Width measuresOFFSET
    Offset, Amount is offset of new edges from original.WIDTH Width, Amount
    is width of new face.DEPTH Depth, Amount is perpendicular distance from
    original edge to bevel face.PERCENT Percent, Amount is percent of
    adjacent edge length.
    :type offset_type: Union[str, int]
    ...
    :param clamp_overlap: Clamp Overlap, Do not allow beveled edges/vertices
    ...
    :param material: Material, Material for bevel faces (-1 means use
    adjacent faces)
    :type material: int
    :param harden_normals: Harden Normals, Match normals of new faces to
    adjacent faces
    :type harden_normals: bool
    :param face_strength_mode: Face Strength Mode, Whether to set face
    strength, and which faces to set face strength onNONE None, Do not set
    face strength.NEW New, Set face strength on new faces only.AFFECTED
    Affected, Set face strength on new and modified faces only.ALL All,
    Set face strength on all faces.
    :type face_strength_mode: Union[str, int]
    :param miter_outer: Outer Miter, Pattern to use for outside of
    mitersSHARP Sharp, Outside of miter is sharp.PATCH Patch, Outside of
    miter is squared-off patch.ARC Arc, Outside of miter is arc.
    :type miter_outer: Union[str, int]
    :param miter_inner: Inner Miter, Pattern to use for inside of mitersSHARP
    Sharp, Inside of miter is sharp.ARC Arc, Inside of miter is arc.
    :type miter_inner: Union[str, int]
    :param spread: Spread, Amount to spread arcs for arc inner miters 
    :type spread: float
    :param use_custom_profile: Custom Profile, Use a custom profile for the
    bevel
    :type use_custom_profile: bool
    :param vmesh_method: Vertex Mesh Method, The method to use to create
    meshes at intersectionsADJ Grid Fill, Default patterned fill.CUTOFF
    Cutoff, A cut-off at each profile’s end before the intersection.
    :type vmesh_method: Union[str, int]
...
def bisect(plane_co: float = (0.0, 0.0, 0.0),
    ...
    :param threshold: Axis Threshold, Preserves the existing geometry along
    the cut plane
    :type threshold: float
...
def blend_from_shape(shape: Union[str, int] = '',
                     blend: float = 1.0,
                     add: bool = True):
    """Blend in shape from a shape key 

    :param shape: Shape, Shape key to use for blending 
    :type shape: Union[str, int]
...
def bridge_edge_loops(type: Union[str, int] = 'SINGLE',
                      use_merge: bool = False,
                      merge_factor: float = 0.5,
                      twist_offset: int = 0,
                      number_cuts: int = 0,
                      interpolation: Union[str, int] = 'PATH',
                      smoothness: float = 1.0,
                      profile_shape_factor: float = 0.0,
                      profile_shape: Union[str, int] = 'SMOOTH'):
    ...
    :type interpolation: Union[str, int]
    :param smoothness: Smoothness, Smoothness factor 
    :type smoothness: float
    :param profile_shape_factor: Profile Factor, How much intermediary new
    edges are shrunk/expanded
    :type profile_shape_factor: float
    :param profile_shape: Profile Shape, Shape of the profileSMOOTH Smooth,
    Smooth falloff.SPHERE Sphere, Spherical falloff.ROOT Root,
    Root falloff.INVERSE_SQUARE Inverse Square, Inverse Square falloff.SHARP
    Sharp, Sharp falloff.LINEAR Linear, Linear falloff.
    :type profile_shape: Union[str, int]
    """
...
def convex_hull(delete_unused: bool = True,
    ...
    :param delete_unused: Delete Unused, Delete selected elements that are
    not used by the hull
    :type delete_unused: bool
    :param use_existing_faces: Use Existing Faces, Skip hull triangles that
    are covered by a pre-existing face
    :type use_existing_faces: bool
    :param make_holes: Make Holes, Delete selected faces that are used by the
    hull
...
def decimate(ratio: float = 1.0,
             ...
             symmetry_axis: Union[str, int] = 'Y'):
    """Simplify geometry by collapsing edges 

    :param ratio: Ratio 
    :type ratio: float
    :param use_vertex_group: Vertex Group, Use active vertex group as an
    influence
    ...
    :type symmetry_axis: Union[str, int]
    """

    pass


def delete(type: Union[str, int] = 'VERT'):
...
def delete_edgeloop(use_face_split: bool = True):
    ...
    :param use_face_split: Face Split, Split off face corners to maintain
...
def dissolve_degenerate(threshold: float = 0.0001):
    ...
    :param threshold: Merge Distance, Maximum distance between elements to
    merge
    :type threshold: float
    """
...
def dissolve_limited(angle_limit: float = 0.0872665,
                     ...
                     delimit: Set[Union[str, int]] = {'NORMAL'}):
    """Dissolve selected edges and verts, limited by the angle of surrounding
    geometry

    :param angle_limit: Max Angle, Angle limit 
    :type angle_limit: float
    :param use_dissolve_boundaries: All Boundaries, Dissolve all vertices in
    between face boundaries
    :type use_dissolve_boundaries: bool
    :param delimit: Delimit, Delimit dissolve operationNORMAL Normal, Delimit
    by face directions.MATERIAL Material, Delimit by face material.SEAM Seam,
    Delimit by edge seams.SHARP Sharp, Delimit by sharp edges.UV UVs, Delimit
    by UV coordinates.
    :type delimit: Set[Union[str, int]]
    """
...
def dissolve_mode(use_verts: bool = False,
    ...
    surrounding geometry
    :type use_face_split: bool
    :param use_boundary_tear: Tear Boundary, Split off face corners instead
    of merging faces
...
def dupli_extrude_cursor(rotate_source: bool = True):
    """Duplicate and extrude selected vertices, edges or faces towards the
    mouse cursor

    :param rotate_source: Rotate Source, Rotate initial selection giving
    better shape
...
def duplicate_move(MESH_OT_duplicate=None, TRANSFORM_OT_translate=None):
    ...
    :param MESH_OT_duplicate: Duplicate, Duplicate selected vertices,
    edges or faces
...
def extrude_edges_move(MESH_OT_extrude_edges_indiv=None,
    ...
    :param MESH_OT_extrude_edges_indiv: Extrude Only Edges, Extrude
    individual edges only
    :param TRANSFORM_OT_translate: Move, Move selected items 
    """
...
def extrude_faces_move(MESH_OT_extrude_faces_indiv=None,
    ...
    :param MESH_OT_extrude_faces_indiv: Extrude Individual Faces, Extrude
    individual faces only
    :param TRANSFORM_OT_shrink_fatten: Shrink/Fatten, Shrink/fatten selected
    vertices along normals
    """
...
def extrude_vertices_move(MESH_OT_extrude_verts_indiv=None,
    ...
    :param MESH_OT_extrude_verts_indiv: Extrude Only Vertices, Extrude
    individual vertices only
    :param TRANSFORM_OT_translate: Move, Move selected items 
    """
...
def faces_mirror_uv(direction: Union[str, int] = 'POSITIVE',
                    precision: int = 3):
...
def fill_grid(span: int = 1, offset: int = 0, use_interp_simple: bool = False):
    ...
    :type offset: int
    :param use_interp_simple: Simple Blending, Use simple interpolation of
    grid vertices
...
def fill_holes(sides: int = 4):
    ...
    :param sides: Sides, Number of sides in hole required to fill (zero fills
    all holes)
    :type sides: int
    """
...
def inset(use_boundary: bool = True,
    ...
    :param use_relative_offset: Offset Relative, Scale the offset by
    surrounding geometry
...
def intersect(mode: Union[str, int] = 'SELECT_UNSELECT',
              separate_mode: Union[str, int] = 'CUT',
              threshold: float = 1e-06):
    """Cut an intersection into faces 

    :param mode: SourceSELECT Self Intersect, Self intersect selected
    faces.SELECT_UNSELECT Selected/Unselected, Intersect selected with
    unselected faces.
    :type mode: Union[str, int]
    :param separate_mode: Separate ModeALL All, Separate all geometry from
    intersections.CUT Cut, Cut into geometry keeping each side separate (
    Selected/Unselected only).NONE Merge, Merge all geometry from the
    intersection.
    :type separate_mode: Union[str, int]
...
def intersect_boolean(operation: Union[str, int] = 'DIFFERENCE',
                      use_swap: bool = False,
                      threshold: float = 1e-06):
    ...
    :param use_swap: Swap, Use with difference intersection to swap which
    side is kept
    :type use_swap: bool
    :param threshold: Merge threshold 
    :type threshold: float
...
def knife_project(cut_through: bool = False):
    ...
    :param cut_through: Cut through, Cut through all faces, not just visible
    ones
...
def knife_tool(use_occlude_geometry: bool = True,
    ...
    :param use_occlude_geometry: Occlude Geometry, Only cut the front most
    geometry
...
def loop_to_region(select_bigger: bool = False):
    ...
    :param select_bigger: Select Bigger, Select bigger regions instead of
    smaller ones
...
def loopcut(number_cuts: int = 1,
            ...
            falloff: Union[str, int] = 'INVERSE_SQUARE',
            object_index: int = -1,
            edge_index: int = -1,
    ...
    :param smoothness: Smoothness, Smoothness factor 
    :type smoothness: float
    :param falloff: Falloff, Falloff type the featherSMOOTH Smooth, Smooth
    falloff.SPHERE Sphere, Spherical falloff.ROOT Root,
    Root falloff.INVERSE_SQUARE Inverse Square, Inverse Square falloff.SHARP
    Sharp, Sharp falloff.LINEAR Linear, Linear falloff.
    :type falloff: Union[str, int]
...
def mark_sharp(clear: bool = False, use_verts: bool = False):
    ...
    :param clear: Clear 
    :type clear: bool
    :param use_verts: Vertices, Consider vertices instead of edges to select
...
def merge(type: Union[str, int] = 'CENTER', uvs: bool = False):
...
def mod_weighted_strength(set: bool = False,
                          face_strength: Union[str, int] = 'MEDIUM'):
    """Set/Get strength of face (used in Weighted Normal modifier) 

    :param set: Set value, Set Value of faces 
    :type set: bool
    :param face_strength: Face Strength, Strength to use for assigning or
    selecting face influence for weighted normal modifier
    :type face_strength: Union[str, int]
    """
...
def normals_tools(mode: Union[str, int] = 'COPY',
                  absolute: bool = False):
    """Custom normals tools using Normal Vector of UI 

    :param mode: Mode, Mode of tools taking input from InterfaceCOPY Copy
    Normal, Copy normal to buffer.PASTE Paste Normal, Paste normal from
    buffer.ADD Add Normal, Add normal vector with selection.MULTIPLY Multiply
    Normal, Multiply normal vector with selection.RESET Reset Normal,
    Reset buffer and/or normal of selected element.
    :type mode: Union[str, int]
    :param absolute: Absolute Coordinates, Copy Absolute coordinates or
    Normal vector
    :type absolute: bool
    """
...
def offset_edge_loops_slide(MESH_OT_offset_edge_loops=None,
    ...
    :param MESH_OT_offset_edge_loops: Offset Edge Loop, Create offset edge
    loop from the current selection
    :param TRANSFORM_OT_edge_slide: Edge Slide, Slide an edge loop along a mesh 
    """
...
def paint_mask_extract(mask_threshold: float = 0.5,
    ...
    :param mask_threshold: Threshold, Minimum mask value to consider the
    vertex valid to extract a face from the original mesh
    :type mask_threshold: float
    :param add_boundary_loop: Add Boundary Loop, Add an extra edge loop to
    better preserve the shape when applying a subdivision surface modifier
    :type add_boundary_loop: bool
    :param smooth_iterations: Smooth Iterations, Smooth iterations applied to
    the extracted mesh
    :type smooth_iterations: int
    :param apply_shrinkwrap: Project to Sculpt, Project the extracted mesh
    into the original sculpt
    :type apply_shrinkwrap: bool
    :param add_solidify: Extract as Solid, Extract the mask as a solid object
    with a solidify modifier
...
def paint_mask_slice(mask_threshold: float = 0.5,
    ...
    :param new_object: Slice to New Object, Create a new object from the
    sliced mask
...
def point_normals(mode: Union[str, int] = 'COORDINATES',
                  invert: bool = False,
                  align: bool = False,
    ...
    :param mode: Mode, How to define coordinates to point custom normals
    toCOORDINATES Coordinates, Use static coordinates (defined by various
    means).MOUSE Mouse, Follow mouse cursor.
    :type mode: Union[str, int]
    ...
    :type spherize: bool
    :param spherize_strength: Spherize Strength, Ratio of spherized normal to
    original normal
...
def poke(offset: float = 0.0,
         use_relative_offset: bool = False,
         center_mode: Union[str, int] = 'MEDIAN_WEIGHTED'):
    ...
    surrounding geometry
    :type use_relative_offset: bool
    :param center_mode: Poke Center, Poke Face Center
    CalculationMEDIAN_WEIGHTED Weighted Median, Weighted median face
    center.MEDIAN Median, Median face center.BOUNDS Bounds, Face bounds center.
    :type center_mode: Union[str, int]
    """
...
def polybuild_delete_at_cursor(
        ...
        proportional_edit_falloff: Union[str, int] = 'SMOOTH',
    ...
    :type proportional_edit_falloff: Union[str, int]
...
def polybuild_extrude_at_cursor_move(
    ...
    :param MESH_OT_polybuild_transform_at_cursor: Poly Build Transform at
    Cursor
    :param MESH_OT_extrude_edges_indiv: Extrude Only Edges, Extrude
    individual edges only
    :param TRANSFORM_OT_translate: Move, Move selected items 
...
def polybuild_face_at_cursor(
    ...
    :param create_quads: Create quads, Automatically split edges in triangles
    to maintain quad topology
...
def primitive_circle_add(vertices: int = 32,
                         radius: float = 1.0,
                         fill_type: Union[str, int] = 'NOTHING',
                         calc_uvs: bool = True,
    ...
    :param radius: Radius 
    :type radius: float
    :param fill_type: Fill TypeNOTHING Nothing, Don’t fill at all.NGON Ngon,
    Use ngons.TRIFAN Triangle Fan, Use triangle fans.
    :type fill_type: Union[str, int]
...
def primitive_cone_add(vertices: int = 32,
                       ...
                       radius2: float = 0.0,
                       depth: float = 2.0,
                       end_fill_type: Union[str, int] = 'NGON',
                       calc_uvs: bool = True,
                       enter_editmode: bool = False,
                       align: Union[str, int] = 'WORLD',
                       location: float = (0.0, 0.0, 0.0),
    ...
    :param depth: Depth
    :type depth: float
    :param end_fill_type: Base Fill TypeNOTHING Nothing, Don’t fill at
    all.NGON Ngon, Use ngons.TRIFAN Triangle Fan, Use triangle fans.
    :type end_fill_type: Union[str, int]
    :param calc_uvs: Generate UVs, Generate a default UV map
    :type calc_uvs: bool
    :param enter_editmode: Enter Editmode, Enter editmode when adding this
    object
...
def primitive_cube_add(size: float = 2.0,
                       ...
                       rotation: tuple = (0.0, 0.0, 0.0)) -> object:
...
def primitive_cylinder_add(vertices: int = 32,
    ...
    :param depth: Depth
    :type depth: float
    :param end_fill_type: Cap Fill TypeNOTHING Nothing, Don’t fill at
    all.NGON Ngon, Use ngons.TRIFAN Triangle Fan, Use triangle fans.
    :type end_fill_type: Union[str, int]
    :param calc_uvs: Generate UVs, Generate a default UV map
    :type calc_uvs: bool
...
def primitive_torus_add(align: Union[str, int] = 'WORLD',
                        location: Tuple[float, float, float] = (0.0, 0.0, 0.0),
                        rotation: Tuple[float, float, float] = (0.0, 0.0, 0.0),
                        major_segments: int = 48,
                        minor_segments: int = 12,
                        mode: Union[str, int] = 'MAJOR_MINOR',
    ...
    :param align: AlignWORLD World, Align the new object to the world.VIEW
    View, Align the new object to the view.CURSOR 3D Cursor, Use the 3D
    cursor orientation for the new object.
    :type align: Union[str, int]
    :param location: Location 
    :type location: float
    :param rotation: Rotation 
    :type rotation: float
    :param major_segments: Major Segments, Number of segments for the main
    ring of the torus
    :type major_segments: int
    :param minor_segments: Minor Segments, Number of segments for the minor
    ring of the torus
    :type minor_segments: int
    :param mode: Torus DimensionsMAJOR_MINOR Major/Minor, Use the major/minor
    radii for torus dimensions.EXT_INT Exterior/Interior, Use the
    exterior/interior radii for torus dimensions.
    :type mode: Union[str, int]
    :param major_radius: Major Radius, Radius from the origin to the center
...
def quads_convert_to_tris(quad_method: Union[str, int] = 'BEAUTY',
                          ngon_method: Union[str, int] = 'BEAUTY'):
    ...
    :type quad_method: Union[str, int]
    :param ngon_method: Polygon Method, Method for splitting the polygons
    into trianglesBEAUTY Beauty, Arrange the new triangles evenly (slow).CLIP
    Clip, Split the polygons with an ear clipping algorithm.
    :type ngon_method: Union[str, int]
    """
...
def rip_edge_move(MESH_OT_rip_edge=None, TRANSFORM_OT_translate=None):
    ...
    :param MESH_OT_rip_edge: Extend Vertices, Extend vertices along the edge
...
def select_axis(orientation: Union[str, int] = 'LOCAL',
                sign: Union[str, int] = 'POS',
                axis: Union[str, int] = 'X',
                threshold: float = 0.0001):
    """Select all data in the mesh on a single axis 

    :param orientation: Axis Mode, Axis orientationGLOBAL Global, Align the
...
def select_less(use_face_step: bool = True):
    """Deselect vertices, edges or faces at the boundary of each selection
    region
...
def select_linked(delimit: Set[Union[str, int]] = {'SEAM'}):
    """Select all vertices connected to the current selection 

    :param delimit: Delimit, Delimit selected regionNORMAL Normal, Delimit by
    face directions.MATERIAL Material, Delimit by face material.SEAM Seam,
    Delimit by edge seams.SHARP Sharp, Delimit by sharp edges.UV UVs, Delimit
    by UV coordinates.
    :type delimit: Set[Union[str, int]]
    """
...
def select_linked_pick(deselect: bool = False,
                       delimit: Set[Union[str, int]] = {'SEAM'},
                       index=-1):
...
def select_mirror(axis: Set[Union[str, int]] = {'X'},
                  extend: bool = False):
    """Select mesh items at mirrored locations 

    :param axis: Axis 
    :type axis: Set[Union[str, int]]
...
def select_mode(use_extend: bool = False,
                ...
                type: Union[str, int] = 'VERT',
                action: Union[str, int] = 'TOGGLE'):
    ...
    :param type: TypeVERT Vertex, Vertex selection mode.EDGE Edge,
    Edge selection mode.FACE Face, Face selection mode.
    :type type: Union[str, int]
    :param action: Action, Selection action to executeDISABLE Disable,
    Disable selected markers.ENABLE Enable, Enable selected markers.TOGGLE
...
def select_non_manifold(extend: bool = True,
    ...
    :param use_non_contiguous: Non Contiguous, Edges between faces pointing
...
def select_nth(skip: int = 1, nth: int = 1, offset: int = 0):
    ...
    :param skip: Deselected, Number of deselected elements in the repetitive
    sequence
    :type skip: int
    :param nth: Selected, Number of selected elements in the repetitive
    sequence
    :type nth: int
...
def select_similar(type: Union[str, int] = 'NORMAL',
                   compare: Union[str, int] = 'EQUAL',
                   threshold: float = 0.0):
...
def separate(type: Union[str, int] = 'SELECTED'):
...
def shortest_path_pick(edge_mode: Union[str, int] = 'SELECT',
    ...
    :param edge_mode: Edge Tag, The edge flag to tag when selecting the
    shortest path
    :type edge_mode: Union[str, int]
    :param use_face_step: Face Stepping, Traverse connected faces (includes
    diagonals and edge-rings)
    :type use_face_step: bool
    :param use_topology_distance: Topology Distance, Find the minimum number
    of steps, ignoring spatial distance
    :type use_topology_distance: bool
    :param use_fill: Fill Region, Select all paths between the
...
def shortest_path_select(edge_mode: Union[str, int] = 'SELECT',
                         use_face_step: bool = False,
                         use_topology_distance: bool = False,
                         use_fill: bool = False,
                         skip: int = 0,
...
def sort_elements(type: Union[str, int] = 'VIEW_ZAXIS',
                  elements: Set[Union[str, int]] = {'VERT'},
                  reverse: bool = False,
                  seed: int = 0):
    """The order of selected vertices/edges/faces is modified, based on a
    given method

    :param type: Type, Type of re-ordering operation to applyVIEW_ZAXIS View
    Z Axis, Sort selected elements from farthest to nearest one in current
    view.VIEW_XAXIS View X Axis, Sort selected elements from left to right
    one in current view.CURSOR_DISTANCE Cursor Distance, Sort selected
    elements from nearest to farthest from 3D cursor.MATERIAL Material,
    Sort selected elements from smallest to greatest material index (faces
    only!).SELECTED Selected, Move all selected elements in first places,
    preserving their relative order (WARNING: this will affect unselected
    elements’ indices as well!).RANDOMIZE Randomize, Randomize order of
    selected elements.REVERSE Reverse, Reverse current order of selected
    elements.
    :type type: Union[str, int]
    :param elements: Elements, Which elements to affect (vertices,
    edges and/or faces)
    :type elements: Set[Union[str, int]]
...
def spin(steps: int = 9,
    ...
    :type angle: float
    :param use_auto_merge: Auto Merge, Merge first/last when the angle is a
    full revolution
...
def subdivide(number_cuts: int = 1,
              smoothness: float = 0.0,
              ngon: bool = True,
              quadcorner: Union[str, int] = 'STRAIGHT_CUT',
              fractal: float = 0.0,
    ...
    :param smoothness: Smoothness, Smoothness factor 
    :type smoothness: float
    :param ngon: Create N-Gons, When disabled, newly created faces are
    limited to 3-4 sided faces
    :type ngon: bool
    :param quadcorner: Quad Corner Type, How to subdivide quad corners (
    anything other than Straight Cut will prevent ngons)
    :type quadcorner: Union[str, int]
    :param fractal: Fractal, Fractal randomness factor 
    :type fractal: float
    :param fractal_along_normal: Along Normal, Apply fractal displacement
    along normal only
...
def symmetrize(direction: Union[str, int] = 'NEGATIVE_X',
               threshold: float = 0.0001):
    """Enforce symmetry (both form and topological) across an axis 

    :param direction: Direction, Which sides to copy from and to 
    :type direction: Union[str, int]
    :param threshold: Threshold, Limit for snap middle vertices to the axis
    center
    :type threshold: float
    """
...
def symmetry_snap(direction: Union[str, int] = 'NEGATIVE_X',
    ...
    :param direction: Direction, Which sides to copy from and to 
    :type direction: Union[str, int]
    :param threshold: Threshold, Distance within which matching vertices are
    searched
    :type threshold: float
...
def vertices_smooth_laplacian(repeat: int = 1,
    ...
    :type use_z: bool
    :param preserve_volume: Preserve Volume, Apply volume preservation after
    smooth
