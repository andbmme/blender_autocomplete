...
def add_and_link_node(type: str = "",
    ...
    :type type: str
    :param use_transform: Use Transform, Start transform operator after
    inserting the node
...
def duplicate_move(NODE_OT_duplicate=None, NODE_OT_translate_attach=None):
    ...
    :param NODE_OT_translate_attach: Move and Attach, Move nodes and attach
    to frame
    """
...
def group_separate(type: typing.Union[str, int] = 'COPY'):
    ...
    :param type: TypeCOPY Copy, Copy to parent node tree, keep group
    intact.MOVE Move, Move to parent node tree, remove from group.
...
def move_detach_links(NODE_OT_links_detach=None,
    ...
    :param NODE_OT_links_detach: Detach Links, Remove all links to selected
    nodes, and try to connect neighbor nodes together
    :param TRANSFORM_OT_translate: Move, Move selected items 
    :param NODE_OT_insert_offset: Insert Offset, Automatically offset nodes
    on insertion
    """
...
def select_box(tweak: bool = False,
    ...
    :param tweak: Tweak, Only activate when mouse is not over a node - useful
    for tweak gesture
    :type tweak: bool
    :param xmin: X Min 
    :type xmin: int
