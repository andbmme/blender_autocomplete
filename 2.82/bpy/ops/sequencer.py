...
def change_effect_type(type: typing.Union[str, int] = 'CROSS'):
    ...
    :param type: Type, Sequencer effect typeCROSS Crossfade, Crossfade effect
    strip type.ADD Add, Add effect strip type.SUBTRACT Subtract, Subtract
    effect strip type.ALPHA_OVER Alpha Over, Alpha Over effect strip
    type.ALPHA_UNDER Alpha Under, Alpha Under effect strip type.GAMMA_CROSS
    Gamma Cross, Gamma Cross effect strip type.MULTIPLY Multiply, Multiply
    effect strip type.OVER_DROP Alpha Over Drop, Alpha Over Drop effect strip
    type.WIPE Wipe, Wipe effect strip type.GLOW Glow, Glow effect strip
    type.TRANSFORM Transform, Transform effect strip type.COLOR Color,
    Color effect strip type.SPEED Speed, Color effect strip type.MULTICAM
    Multicam Selector.ADJUSTMENT Adjustment Layer.GAUSSIAN_BLUR Gaussian
    Blur.TEXT Text.COLORMIX Color Mix.
...
def change_path(filepath: str = "",
    ...
    :param use_placeholders: Use Placeholders, Use placeholders for missing
    frames of the strip
...
def duplicate_move(SEQUENCER_OT_duplicate=None, TRANSFORM_OT_seq_slide=None):
    ...
    :param SEQUENCER_OT_duplicate: Duplicate Strips, Duplicate the selected
    strips
    :param TRANSFORM_OT_seq_slide: Sequence Slide, Slide a sequence strip in
    time
    """
...
def effect_strip_add(frame_start: int = 0,
    ...
    Blur.TEXT Text.COLORMIX Color Mix.
    :type type: typing.Union[str, int]
    :param color: Color, Initialize the strip with this color (only used when
    type=’COLOR’)
    :type color: float
    """
...
def fades_add(duration_seconds: float = 1.0,
    ...
    :param type: Fade type, Fade in, out, both in and out, to, or from the
    playhead. Default is both in and outIN_OUT Fade In And Out, Fade selected
    strips in and out.IN Fade In, Fade in selected strips.OUT Fade Out,
    Fade out selected strips.CURSOR_FROM From Playhead, Fade from the time
    cursor to the end of overlapping sequences.CURSOR_TO To Playhead,
    Fade from the start of sequences under the time cursor to the current
    frame.
    :type type: typing.Union[str, int]
    """
...
def gap_insert(frames: int = 10):
    """Insert gap at current frame to first strips at the right, independent
    of selection or locked state of strips
...
def gap_remove(all: bool = False):
    """Remove gap at current frame to first strip at the right, independent
    of selection or locked state of strips
...
def movie_strip_add(
    ...
    :type sound: bool
    :param use_framerate: Use Movie Framerate, Use framerate from the movie
    to keep sound and video in sync
...
def reload(adjust_length: bool = False):
    ...
    :param adjust_length: Adjust Length, Adjust length of strips to their
    data length
...
def select(wait_to_deselect_others: bool = False,
    ...
    :param linked_handle: Linked Handle, Select handles next to the active
    strip
    :type linked_handle: bool
    :param left_right: Left/Right, Select based on the current frame side the
    cursor is onNONE None, Don’t do left-right selection.MOUSE Mouse,
    Use mouse position for selection.LEFT Left, Select left.RIGHT Right,
    Select right.
...
def select_grouped(type: typing.Union[str, int] = 'TYPE',
    ...
    :param type: TypeTYPE Type, Shared strip type.TYPE_BASIC Global Type,
    All strips of same basic type (Graphical or Sound).TYPE_EFFECT Effect
    Type, Shared strip effect type (if active strip is not an effect one,
    select all non-effect strips).DATA Data, Shared data (scene, image,
    sound, etc.).EFFECT Effect, Shared effects.EFFECT_LINK Effect/Linked,
    Other strips affected by the active one (sharing some time, and below or
    effect-assigned).OVERLAP Overlap, Overlapping time.
    :type type: typing.Union[str, int]
    :param extend: Extend, Extend selection instead of deselecting everything
    first
    :type extend: bool
    :param use_active_channel: Same Channel, Only consider strips on the same
    channel as the active one
...
def strip_modifier_copy(type: typing.Union[str, int] = 'REPLACE'):
    ...
    :param type: TypeREPLACE Replace, Replace modifiers in destination.APPEND
...
def strip_modifier_move(name: str = "Name",
    ...
    :type name: str
    :param direction: TypeUP Up, Move modifier up in the stack.DOWN Down,
...
def unmute(unselected: bool = False):
    ...
    :param unselected: Unselected, Unmute unselected rather than selected
    strips
    :type unselected: bool
    """
...
def view_zoom_ratio(ratio: float = 1.0):
    ...
    :param ratio: Ratio, Zoom ratio, 1.0 is 1:1, higher is zoomed in,
    lower is zoomed out
    :type ratio: float
    """
