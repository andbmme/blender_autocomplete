...
def action_pushdown(channel_index: int = -1):
    ...
    :param channel_index: Channel Index, Index of NLA action channel to
    perform pushdown operation on
...
def action_sync_length(active: bool = True):
    ...
    :param active: Active Strip Only, Only sync the active length for the
    active strip
    :type active: bool
    """
...
def action_unlink(force_delete: bool = False):
    ...
    :param force_delete: Force Delete, Clear Fake User and remove copy
    stashed in this datablock’s NLA stack
...
def bake(frame_start: int = 1,
    ...
    :type step: int
    :param only_selected: Only Selected Bones, Only key selected bones (Pose
    baking only)
    :type only_selected: bool
    :param visual_keying: Visual Keying, Keyframe from the final
    transformations (with constraints applied)
    :type visual_keying: bool
    :param clear_constraints: Clear Constraints, Remove all constraints from
    keyed object/bones, and do ‘visual’ keying
    :type clear_constraints: bool
    :param clear_parents: Clear Parents, Bake animation onto the object then
    clear parents (objects only)
    :type clear_parents: bool
    :param use_current_action: Overwrite Current Action, Bake animation into
    current action, instead of creating a new one (useful for baking only
    part of bones in an armature)
    :type use_current_action: bool
    :param bake_types: Bake Data, Which data’s transformations to bakePOSE
    Pose, Bake bones transformations.OBJECT Object, Bake object
    transformations.
...
def duplicate(linked: bool = False,
    ...
    """Duplicate selected NLA-Strips, adding the new strips in new tracks
    above the originals

    :param linked: Linked, When duplicating strips, assign new copies of the
    actions they use
    :type linked: bool
    :param mode: Mode 
...
def fmodifier_add(type: typing.Union[str, int] = 'NULL',
    ...
    :param type: TypeNULL Invalid.GENERATOR Generator, Generate a curve using
    a factorized or expanded polynomial.FNGENERATOR Built-In Function,
    Generate a curve using standard math functions such as sin and
    cos.ENVELOPE Envelope, Reshape F-Curve values - e.g. change amplitude of
    movements.CYCLES Cycles, Cyclic extend/repeat keyframe sequence.NOISE
    Noise, Add pseudo-random noise on top of F-Curves.LIMITS Limits, Restrict
    maximum and minimum values of F-Curve.STEPPED Stepped Interpolation,
    Snap values to nearest grid-step - e.g. for a stop-motion look.
    :type type: typing.Union[str, int]
    :param only_active: Only Active, Only add a F-Modifier of the specified
...
def fmodifier_paste(only_active: bool = True, replace: bool = False):
    ...
    :type only_active: bool
    :param replace: Replace Existing, Replace existing F-Modifiers, instead
    of just appending to the end of the existing list
    :type replace: bool
    """
...
def tracks_add(above_selected: bool = False):
    ...
    :param above_selected: Above Selected, Add a new NLA Track above every
...
def tweakmode_enter(isolate_action: bool = False):
    ...
    :param isolate_action: Isolate Action, Enable ‘solo’ on the NLA Track
    containing the active strip, to edit it without seeing the effects of the
    NLA stack
...
def tweakmode_exit(isolate_action: bool = False):
    ...
    :param isolate_action: Isolate Action, Disable ‘solo’ on any of the NLA
    Tracks after exiting tweak mode to get things back to normal
    :type isolate_action: bool
    """
