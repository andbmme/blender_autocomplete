...
def abspath(path,
    ...
    :param start: Relative to this path, when not set the current filename is
    used.
...
def display_name(name, *, has_ext=True):
    """Creates a display string from name to be used menus and the user
    interface. Capitalize the first letter in all lowercase names, mixed case
    names are kept as is. Intended for use with filenames and module names.
...
def display_name_from_filepath(name):
    """Returns the path stripped of directory and extension, ensured to be
    utf8 compatible.
...
def display_name_to_filepath(name):
    """Performs the reverse of display_name using literal versions of
    characters which aren’t supported in a filepath.
...
def ensure_ext(filepath, ext: str, case_sensitive: bool = False):
    ...
    :param ext: The extension to check for, can be a compound extension.
    Should start with a dot, such as ‘.blend’ or ‘.tar.gz’.
    :type ext: str
...
def is_subdir(path: typing.Union[bytes, str], directory):
    """Returns true if path in a subdirectory of directory. Both paths must
    be absolute.
...
def reduce_dirs(dirs: list) -> list:
    """Given a sequence of directories, remove duplicates and any directories
    nested in one of the other paths. (Useful for recursive path searching).
...
def relpath(path: typing.Union[bytes, str],
            start: typing.Union[bytes, str] = None):
    """Returns the path relative to the current blend file using the “//”
    prefix.
...
def resolve_ncase(path):
    """Resolve a case insensitive path on a case sensitive system, returning
    a string with the path if found else return the original path.
