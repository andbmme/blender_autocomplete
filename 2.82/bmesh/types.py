...
class BMDeformVert:
    ...
    def get(self, key: int, default=None):
        """Returns the deform weight matching the key or default when not
        found (matches pythons dictionary function of the same name).

        :param key: The key associated with deform weight. 
        :type key: int
        :param default: Optional argument for the value to return if key is
        not found.
        :type default: 
        """
        pass
    ...
    def items(self, ) -> list:
        """Return (group, weight) pairs for this vertex (matching pythons
        dict.items() functionality).
    ...
    def keys(self, ) -> list:
        """Return the group indices used by this vertex (matching pythons
        dict.keys() functionality).
    ...
    def values(self, ) -> list:
        """Return the weights of the deform vertex (matching pythons
        dict.values() functionality).
...
class BMEdge:
    ...
    """True when this edge is manifold, between two faces with the same 
    winding (read-only). 
    ...
    is_convex: bool = None
    """True when this edge joins two convex faces, depends on a valid face 
    normal (read-only). 
    ...
    def calc_face_angle_signed(self, fallback=None) -> float:
        ...
        :type fallback: 
        :rtype: float
        :return:  The angle between 2 connected faces in radians (negative
        for concave join).
        """
        pass
    ...
    def calc_tangent(self, loop: 'BMLoop') -> 'mathutils.Vector':
        """Return the tangent at this edge relative to a face (pointing
        inward into the face). This uses the face normal for calculation.
    ...
    def hide_set(self, hide: bool):
        """Set the hide state. This is different from the hide attribute
        because it updates the selection and hide state of associated geometry.
    ...
    def other_vert(self, vert: 'BMVert') -> 'BMVert':
        """Return the other vertex on this edge or None if the vertex is not
        used by this edge.
    ...
    def select_set(self, select: bool):
        """Set the selection. This is different from the select attribute
        because it updates the selection state of associated geometry.
...
class BMEdgeSeq:
    ...
    def ensure_lookup_table(self, ):
        """This needs to be called again after adding/removing data in this
        sequence.
    ...
    def index_update(self, ):
        """This is the equivalent of looping over all elements and assigning
        the index values.
    ...
    def new(self, verts: 'BMVert', example: 'BMEdge' = None) -> 'BMEdge':
        ...
        :type verts: 'BMVert'
        :param example: Existing edge to initialize settings (optional
        argument).
        :type example: 'BMEdge'
        :rtype: 'BMEdge'
    ...
    def sort(self, key=None, reverse=False):
        """Sort the elements of this sequence, using an optional custom sort
        key. Indices of elements are not changed, BMElemeSeq.index_update()
...
class BMEditSelSeq:
    ...
    def add(self, element):
        """Add an element to the selection history (no action taken if its
        already added).
    ...
    def discard(self, element):
        """Like remove but doesn’t raise an error when the elements not in
        the selection list.
...
class BMElemSeq:
    """When accessed via BMesh.verts, BMesh.edges, BMesh.faces there are also
    functions to create/remomove items. """
...
class BMFace:
    ...
    def copy_from_face_interp(self, face: 'BMFace', vert: bool = True):
        """Interpolate the customdata from another face onto this one (faces
        should overlap).
...
class BMFaceSeq:
    ...
    def new(self, verts: 'BMVert', example: 'BMFace' = None) -> 'BMFace':
        ...
        :type verts: 'BMVert'
        :param example: Existing face to initialize settings (optional
        argument).
        :type example: 'BMFace'
        :rtype: 'BMFace'
        :return:  The newly created face. 
...
class BMIter:
    """Internal BMesh type for looping over verts/faces/edges, used for
    iterating over BMElemSeq types. """
...
class BMLayerCollection:
    """Gives access to a collection of custom-data layers of the same type
    and behaves like python dictionaries, except for the ability to do list
    like index access. """
    ...
    def get(self, key: str, default=None):
        """Returns the value of the layer matching the key or default when
        not found (matches pythons dictionary function of the same name).
    ...
    def items(self, ) -> list:
        """Return the identifiers of collection members (matching pythons
        dict.items() functionality).
    ...
    def values(self, ) -> list:
        """Return the values of collection (matching pythons dict.values()
        functionality).
...
class BMLayerItem:
    """Exposes a single custom data layer, their main purpose is for use as
    item accessors to custom-data when used with vert/edge/face/loop data. """
...
class BMLoop:
    """This is normally accessed from BMFace.loops where each face loop
    ...
    is_convex: bool = None
    """True when this loop is at the convex corner of a face, depends on a 
    valid face normal (read-only). 
    ...
    def calc_angle(self, ) -> float:
        """Return the angle at this loops corner of the face. This is
        calculated so sharper corners give lower angles.
    ...
    def calc_normal(self, ) -> 'mathutils.Vector':
        """Return normal at this loops corner of the face. Falls back to the
        face normal for straight lines.
    ...
    def calc_tangent(self, ) -> 'mathutils.Vector':
        """Return the tangent at this loops corner of the face (pointing
        inward into the face). Falls back to the face normal for straight
        lines.
    ...
    def copy_from_face_interp(self,
                              ...
                              vert: bool = True,
                              multires: bool = True):
        """Interpolate the customdata from a face onto this loop (the loops
        vert should overlap the face).

        :param face: The face to interpolate data from. 
        :type face: 'BMFace'
        :param vert: When enabled, interpolate the loops vertex data (
        optional).
        :type vert: bool
        :param multires: When enabled, interpolate the loops multires data (
        optional).
        :type multires: bool
        """
        pass
...
class BMVert:
    ...
    def calc_shell_factor(self, ) -> float:
        """Return a multiplier calculated based on the sharpness of the
        vertex. Where a flat surface gives 1.0, and higher values sharper
        edges. This is used to maintain shell thickness when offsetting verts
        along their normals.
...
class BMesh:
    ...
    """The selection mode, values can be {‘VERT’, ‘EDGE’, ‘FACE’}, can’t be 
    ...
    def free(self, ):
        """Explicitly free the BMesh data from memory, causing exceptions on
        further access.
    ...
    def from_object(self,
        ...
        """Initialize this bmesh from existing object datablock (currently
    ...
    def transform(self, matrix: 'mathutils.Matrix', filter: set = None):
        ...
        :param filter: set of values in (‘SELECT’, ‘HIDE’, ‘SEAM’, ‘SMOOTH’,
        ‘TAG’).
        :type filter: set
        """
        pass
