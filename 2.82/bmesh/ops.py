...
def average_vert_facedata(bm: 'bmesh.types.BMesh',
    ...
    """ Merge uv/vcols associated with the input vertices at the bounding box
    center. (I know, it’s not averaging but the vert_snap_to_bb_center is
    just too long).
...
def beautify_fill(
        ...
        edges: List['bmesh.types.BMEdge'], use_restrict_tag: bool,
        method: Union[str, int]) -> dict:
    ...
    :return:  geom: new flipped faces and edgestype list of (
...
def bevel(bm: 'bmesh.types.BMesh', geom: List['bmesh.types.BMVert'],
          offset: float, offset_type: Union[str, int], segments: int,
          profile: float, vertex_only: bool, clamp_overlap: bool,
          material: int, loop_slide: bool, mark_seam: bool, mark_sharp: bool,
          harden_normals: bool, face_strength_mode: Union[str, int],
          miter_outer: Union[str, int],
          miter_inner: Union[str, int], spread: float,
          smoothresh: float, use_custom_profile: bool,
          custom_profile: 'bpy.types.bpy_struct',
          vmesh_method: Union[str, int]) -> dict:
    """ Bevels edges and vertices

    :param bm: The bmesh to operate on.
    :type bm: 'bmesh.types.BMesh'
    :param geom: input edges and vertices
    :type geom: List['bmesh.types.BMVert']
    ...
    :param clamp_overlap: do not allow beveled edges/vertices to overlap each
    other
    ...
    :type mark_seam: bool
    :param mark_sharp: extend edge data to allow sharp edges to run across
    bevels
    :type mark_sharp: bool
    :param harden_normals: harden normals
    :type harden_normals: bool
    :param face_strength_mode: whether to set face strength, and which faces
    to set if so
    ...
    :return:  faces: output facestype list of (bmesh.types.BMFace)edges:
    output edgestype list of (bmesh.types.BMEdge)verts: output vertstype list
    of (bmesh.types.BMVert)
    """
...
def bisect_edges(bm: 'bmesh.types.BMesh',
                 edges: List['bmesh.types.BMEdge'], cuts: int,
                 edge_percents: dict) -> dict:
    """ Splits input edges (but doesn’t do anything else). This creates a
    2-valence vert.
    ...
    :return:  geom_split: newly created vertices and edgestype list of (
    bmesh.types.BMVert, bmesh.types.BMEdge, bmesh.types.BMFace)
    """
...
def bisect_plane(
        bm: 'bmesh.types.BMesh', geom: List['bmesh.types.BMVert'],
        dist: float, plane_co: List['mathutils.Vector'],
        plane_no: List['mathutils.Vector'], use_snap_center: bool,
    ...
    :type geom: List['bmesh.types.BMVert']
    :param dist: minimum distance when testing if a vert is exactly on the
    plane
    :type dist: float
    :param plane_co: point on the plane
    :type plane_co: List['mathutils.Vector']
    :param plane_no: direction of the plane
    :type plane_no: List['mathutils.Vector']
    :param use_snap_center: snap axis aligned verts to the center
    :type use_snap_center: bool
    :param clear_outer: when enabled. remove all geometry on the positive
    side of the plane
    :type clear_outer: bool
    :param clear_inner: when enabled. remove all geometry on the negative
    side of the plane
    :type clear_inner: bool
    :return:  geom_cut: output geometry aligned with the plane (new and
    existing)type list of (bmesh.types.BMVert, bmesh.types.BMEdge)geom: input
    and output geometry (result of cut)type list of (bmesh.types.BMVert,
    bmesh.types.BMEdge, bmesh.types.BMFace)
    """
...
def bridge_loops(bm: 'bmesh.types.BMesh',
                 edges: List['bmesh.types.BMEdge'], use_pairs: bool,
    ...
    :return:  faces: new facestype list of (bmesh.types.BMFace)edges: new
    edgestype list of (bmesh.types.BMEdge)
    """

    pass


def collapse(bm: 'bmesh.types.BMesh', edges: List['bmesh.types.BMEdge'],
             uvs: bool):
...
def connect_vert_pair(
        ...
        faces_exclude: List['bmesh.types.BMFace']) -> dict:
...
def connect_verts_concave(bm: 'bmesh.types.BMesh',
                          faces: List['bmesh.types.BMFace']) -> dict:
    ...
    :return:  edges:type list of (bmesh.types.BMEdge)faces:type list of (
    bmesh.types.BMFace)
    """
...
def contextual_create(bm: 'bmesh.types.BMesh',
                      geom: List['bmesh.types.BMVert'], mat_nr: int,
    ...
    :return:  faces: newly-made face(s)type list of (
    bmesh.types.BMFace)edges: newly-made edge(s)type list of (
    bmesh.types.BMEdge)
    """
...
def convex_hull(bm: 'bmesh.types.BMesh',
    ...
    """All hull vertices, faces, and edges are added to ‘geom.out’. Any input
    elements that end up inside the hull (i.e. are not used by an output
    face) are added to the ‘interior_geom’ slot. The ‘unused_geom’ slot will
    contain all interior geometry that is completely unused. Lastly,
    ‘holes_geom’ contains edges and faces that were in the input and are part
    of the hull.

    :param bm: The bmesh to operate on. 
    :type bm: 'bmesh.types.BMesh'
    :param input: Undocumented. 
    :type input: List['bmesh.types.BMVert']
    ...
    bmesh.types.BMFace)geom_interior:type list of (bmesh.types.BMVert,
    bmesh.types.BMEdge, bmesh.types.BMFace)geom_unused:type list of (
    bmesh.types.BMVert, bmesh.types.BMEdge,
    bmesh.types.BMFace)geom_holes:type list of (bmesh.types.BMVert,
    bmesh.types.BMEdge, bmesh.types.BMFace)
    """
...
def create_vert(bm: 'bmesh.types.BMesh',
                co: List['mathutils.Vector']) -> dict:
    """Creates a single vertex; this bmop was necessary for
    click-create-vertex.

    :param bm: The bmesh to operate on. 
    :type bm: 'bmesh.types.BMesh'
    :param co: the coordinate of the new vert 
    :type co: List['mathutils.Vector']
...
def delete(bm: 'bmesh.types.BMesh', geom: List['bmesh.types.BMVert'],
...
def dissolve_edges(bm: 'bmesh.types.BMesh',
                   edges: List['bmesh.types.BMEdge'], use_verts: bool,
...
def duplicate(bm: 'bmesh.types.BMesh', geom: List['bmesh.types.BMVert'],
              dest: 'bmesh.types.BMesh', use_select_history: bool,
              use_edge_flip_from_face: bool) -> dict:
    """Utility operator to duplicate geometry, optionally into a destination
    mesh.
    ...
    bmesh.types.BMFace)geom:type list of (bmesh.types.BMVert,
    bmesh.types.BMEdge, bmesh.types.BMFace)vert_map:type dict mapping
    vert/edge/face types to bmesh.types.BMVert/bmesh.types.BMEdge/bmesh.types
    .BMFaceedge_map:type dict mapping vert/edge/face types to
    bmesh.types.BMVert/bmesh.types.BMEdge/bmesh.types.BMFaceface_map:type
    dict mapping vert/edge/face types to
    bmesh.types.BMVert/bmesh.types.BMEdge/bmesh.types.BMFaceboundary_map:type
    dict mapping vert/edge/face types to
    bmesh.types.BMVert/bmesh.types.BMEdge/bmesh.types.BMFaceisovert_map:type
    dict mapping vert/edge/face types to
    bmesh.types.BMVert/bmesh.types.BMEdge/bmesh.types.BMFace
    """
...
def edgeloop_fill(bm: 'bmesh.types.BMesh',
                  edges: List['bmesh.types.BMEdge'], mat_nr: int,
                  use_smooth: bool) -> dict:
...
def edgenet_prepare(bm: 'bmesh.types.BMesh',
                    edges: List['bmesh.types.BMEdge']) -> dict:
    """Identifies several useful edge loop cases and modifies them so they’ll
    become a face when edgenet_fill is called. The cases covered are:
...
def extrude_edge_only(bm: 'bmesh.types.BMesh',
                      edges: List['bmesh.types.BMEdge'],
                      use_normal_flip: bool, use_select_history: bool) -> dict:
    """Extrudes Edges into faces, note that this is very simple, there’s no
    fancy winged extrusion.
    ...
    :param use_select_history: pass to duplicate 
    :type use_select_history: bool
    :return:  geom: output geometrytype list of (bmesh.types.BMVert,
    bmesh.types.BMEdge, bmesh.types.BMFace)
    """
...
def extrude_face_region(bmesh,
    ...
    :param bmesh: The bmesh to operate on.
    :param geom: edges and faces
    :param edges_exclude: Undocumented.
    :param use_keep_orig: keep original geometry (requires geom to include
    edges).
    :param use_normal_flip: Create faces with reversed direction.
    :param use_normal_from_adjacent: Use winding from surrounding faces
    instead of this region.
    :param use_select_history: pass to duplicate
    :return:  geom
    """
...
def find_doubles(bm: 'bmesh.types.BMesh',
                 verts: List['bmesh.types.BMVert'],
                 keep_verts: List['bmesh.types.BMVert'], dist: float) -> dict:
...
def holes_fill(bm: 'bmesh.types.BMesh',
               edges: List['bmesh.types.BMEdge'], sides: int) -> dict:
...
def inset_region(
        bm: 'bmesh.types.BMesh', faces: List['bmesh.types.BMFace'],
        faces_exclude: List['bmesh.types.BMFace'], use_boundary: bool,
...
def mesh_to_bmesh(bm: 'bmesh.types.BMesh', mesh: 'bpy.types.Mesh',
    ...
    """Load the contents of a mesh into the bmesh. this bmop is private,
    it’s reserved exclusively for entering editmode.
...
def mirror(bm: 'bmesh.types.BMesh', geom: List['bmesh.types.BMVert'],
           matrix: 'mathutils.Matrix', merge_dist: float,
           axis: Union[str, int], mirror_u: bool,
           mirror_v: bool) -> dict:
    ...
    the merge_dist parameter (which defines the minimum distance for welding
    to happen).
...
def planar_faces(bm: 'bmesh.types.BMesh',
                 faces: List['bmesh.types.BMFace'], iterations: int,
                 factor: float) -> dict:
    """Iteratively flatten faces. 

    :param bm: The bmesh to operate on. 
    :param faces: input geometry.
    :param iterations: Number of times to flatten faces (for when connected
    faces are used)
    :param factor: Influence for making planar each iteration
    :return:  geom: output slot, computed boundary geometry.type list of (
    bmesh.types.BMVert, bmesh.types.BMEdge, bmesh.types.BMFace)
    """
...
def pointmerge(bm: 'bmesh.types.BMesh',
               verts: List['bmesh.types.BMVert'],
               merge_co: List['mathutils.Vector']):
...
def poke(bm: 'bmesh.types.BMesh', faces: List['bmesh.types.BMFace'],
         offset: float, center_mode: Union[str, int],
...
def region_extend(bm: 'bmesh.types.BMesh',
                  geom: List['bmesh.types.BMVert'], use_contract: bool,
                  use_faces: bool, use_face_step: bool) -> dict:
    """if use_faces is 0 then geom.out spits out verts and edges, otherwise
    it spits out faces.
...
def remove_doubles(bm: 'bmesh.types.BMesh',
                   verts: List['bmesh.types.BMVert'], dist: float):
...
def rotate(bm: 'bmesh.types.BMesh', cent: List['mathutils.Vector'],
    ...
    :type cent: List['mathutils.Vector']
...
def rotate_colors(bm: 'bmesh.types.BMesh',
                  faces: List['bmesh.types.BMFace'], use_ccw: bool):
...
def scale(bm: 'bmesh.types.BMesh', vec: List['mathutils.Vector'],
          space: 'mathutils.Matrix', verts: List['bmesh.types.BMVert']):
    """Scales vertices by an offset. 

    :param bm: The bmesh to operate on. 
    :type bm: 'bmesh.types.BMesh'
    :param vec: scale factor 
    :type vec: List['mathutils.Vector']
    :param space: matrix to define the space (typically object matrix) 
    :type space: 'mathutils.Matrix'
    :param verts: input vertices 
    :type verts: List['bmesh.types.BMVert']
...
def smooth_laplacian_vert(
    ...
    """Smooths vertices by using Laplacian smoothing propose by. Desbrun,
    et al. Implicit Fairing of Irregular Meshes using Diffusion and Curvature
    Flow.
...
def smooth_vert(bm: 'bmesh.types.BMesh',
                verts: List['bmesh.types.BMVert'], factor: float,
    ...
    :type factor: float
    :param mirror_clip_x: set vertices close to the x axis before the
    operation to 0
    :type mirror_clip_x: bool
    :param mirror_clip_y: set vertices close to the y axis before the
    operation to 0
    :type mirror_clip_y: bool
    :param mirror_clip_z: set vertices close to the z axis before the
    operation to 0
...
def solidify(bm: 'bmesh.types.BMesh', geom: List['bmesh.types.BMVert'],
...
def spin(bm: 'bmesh.types.BMesh', geom: List['bmesh.types.BMVert'],
         cent: List['mathutils.Vector'],
         axis: List['mathutils.Vector'],
         dvec: List['mathutils.Vector'], angle: float,
         space: 'mathutils.Matrix', steps: int, use_merge: bool,
         use_normal_flip: bool, use_duplicate: bool) -> dict:
    """Extrude or duplicate geometry a number of times, rotating and possibly
    translating after each step
    ...
    :type axis: List['mathutils.Vector']
    :param dvec: translation delta per step 
    :type dvec: List['mathutils.Vector']
    ...
    :return:  geom_last: result of last steptype list of (bmesh.types.BMVert,
    bmesh.types.BMEdge, bmesh.types.BMFace)
    """
...
def split(bm: 'bmesh.types.BMesh', geom: List['bmesh.types.BMVert'],
          dest: 'bmesh.types.BMesh', use_only_faces: bool) -> dict:
    """Disconnect geometry from adjacent edges and faces, optionally into a
    destination mesh.
    ...
    :return:  geom:type list of (bmesh.types.BMVert, bmesh.types.BMEdge,
    bmesh.types.BMFace)boundary_map:type dict mapping vert/edge/face types to
    bmesh.types.BMVert/bmesh.types.BMEdge/bmesh.types.BMFaceisovert_map:type
    dict mapping vert/edge/face types to
    bmesh.types.BMVert/bmesh.types.BMEdge/bmesh.types.BMFace
    """
...
def split_edges(
        ...
        verts: List['bmesh.types.BMVert'], use_verts: bool) -> dict:
    ...
    :type verts: List['bmesh.types.BMVert']
    :param use_verts: use ‘verts’ for splitting, else just find verts to
    split from edges
    :type use_verts: bool
    :return:  edges: old output disconnected edgestype list of (
    bmesh.types.BMEdge)
    """
...
def subdivide_edgering(bm: 'bmesh.types.BMesh',
                       edges: List['bmesh.types.BMEdge'],
                       interp_mode: Union[str, int], smooth: float,
                       cuts: int, profile_shape: Union[str, int],
    ...
    :type interp_mode: Union[str, int]
...
def subdivide_edges(
        bm: 'bmesh.types.BMesh', edges: List['bmesh.types.BMEdge'],
        smooth: float, smooth_falloff: Union[str, int], fractal: float,
        along_normal: float, cuts: int, seed: int, custom_patterns: dict,
        edge_percents: dict, quad_corner_type: Union[str, int],
    ...
    :type smooth_falloff: Union[str, int]
    ...
    :type quad_corner_type: Union[str, int]
    :param use_grid_fill: fill in fully-selected faces with a grid 
    :type use_grid_fill: bool
    :param use_single_edge: tessellate the case of one edge selected in a
    quad or triangle
    ...
    :return:  geom_inner:type list of (bmesh.types.BMVert,
    bmesh.types.BMEdge, bmesh.types.BMFace)geom_split:type list of (
    bmesh.types.BMVert, bmesh.types.BMEdge, bmesh.types.BMFace)geom: contains
    all output geometrytype list of (bmesh.types.BMVert, bmesh.types.BMEdge,
    bmesh.types.BMFace)
    """
...
def symmetrize(bm: 'bmesh.types.BMesh',
               input: List['bmesh.types.BMVert'],
               direction: Union[str, int], dist: float) -> dict:
...
def transform(bm: 'bmesh.types.BMesh', matrix: 'mathutils.Matrix',
              space: 'mathutils.Matrix',
              verts: List['bmesh.types.BMVert']):
    """Transforms a set of vertices by a matrix. Multiplies the vertex
...
def triangle_fill(bm: 'bmesh.types.BMesh', use_beauty: bool,
                  use_dissolve: bool, edges: List['bmesh.types.BMEdge'],
                  normal: List['mathutils.Vector']) -> dict:
    ...
    :type normal: List['mathutils.Vector']
    :return:  geom: new faces and edgestype list of (bmesh.types.BMVert,
    bmesh.types.BMEdge, bmesh.types.BMFace)
    """
...
def triangulate(bm: 'bmesh.types.BMesh',
                faces: List['bmesh.types.BMFace'],
                quad_method: Union[str, int],
                ngon_method: Union[str, int]) -> dict:
    """Triangulate. 
    ...
    :return:  edges:type list of (bmesh.types.BMEdge)faces:type list of (
    bmesh.types.BMFace)face_map:type dict mapping vert/edge/face types to
    bmesh.types.BMVert/bmesh.types.BMEdge/bmesh.types.BMFaceface_map_double:
...
def unsubdivide(bm: 'bmesh.types.BMesh',
                verts: List['bmesh.types.BMVert'], iterations: int):
...
def weld_verts(bm: 'bmesh.types.BMesh', targetmap: dict):
    """Welds verts together (kind-of like remove doubles, merge, etc, all of
    which use or will use this bmop). You pass in mappings from vertices to
    the vertices they weld with.
...
def wireframe(bm: 'bmesh.types.BMesh',
              faces: List['bmesh.types.BMFace'], thickness: float,
