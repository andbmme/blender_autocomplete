...
def from_edit_mesh(mesh: 'bpy.types.Mesh') -> 'types.BMesh':
    """ Return a BMesh from this mesh, currently the mesh must already be in
    editmode.
...
def new(use_operators: bool = True) -> 'types.BMesh':
    ...
    :param use_operators: Support calling operators in bmesh.ops (uses some
...
def update_edit_mesh(mesh: 'bpy.types.Mesh',
    ...
    """ Update the mesh after changes to the BMesh in editmode, optionally
    recalculating n-gon tessellation.
