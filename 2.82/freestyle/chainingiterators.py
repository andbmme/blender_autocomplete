...
class ChainPredicateIterator:
    ...
    particular built from a unary predicate and a binary predicate. First,
    the unary predicate is evaluated for all potential next ViewEdges in
    order to only keep the ones respecting a certain constraint. Then,
    the binary predicate is evaluated on the current ViewEdge together with
    each ViewEdge of the previous selection. The first ViewEdge respecting
    both the unary predicate and the binary predicate is kept as the next
    one. If none of the potential next ViewEdge respects these two
    predicates, None is returned. """
    ...
    def __init__(self,
        ...
        the ViewEdges that surround the ending ViewVertex of begin. If false,
        we’ll search over the ViewEdges surrounding the ending ViewVertex of
        begin.
...
class ChainSilhouetteIterator:
    ...
    example, it will follow visible ViewEdges of same nature. As soon,
    as the nature or the visibility changes, the iteration stops (by setting
    the pointed ViewEdge to 0). In the case of an iteration over a set of
    ViewEdge that are both Silhouette and Crease, there will be a precedence
...
class pyChainSilhouetteGenericIterator:
    ...
    def __init__(self,
        ...
        :param stayInSelection: True if it is allowed to go out of the
        selection
...
class pyFillOcclusionsRelativeChainingIterator:
    ...
    def __init__(self, percent: float):
        ...
        :param percent: The maximal length of the occluded part, expressed in
        a percentage of the total chain length.
        :type percent: float
        """
        pass
...
class pySketchyChainSilhouetteIterator:
    ...
    def __init__(self, nRounds: int = 3, stayInSelection: bool = True):
        ...
        :type nRounds: int
        :param stayInSelection: if False, edges outside of the selection can
        be chained.
        :type stayInSelection: bool
        """
        pass
    ...
    def make_sketchy(self, ve):
        """Creates the skeychy effect by causing the chain to run from the
        start again. (loop over itself again)
...
class pySketchyChainingIterator:
    ...
    ViewEdge several times in order to produce multiple strokes per ViewEdge.
    """
