...
def location_3d_to_region_2d(region: 'bpy.types.Region',
    ...
    :param default: Return this value if coord is behind the origin of a
    perspective view.
    :return:  2d location 
    """
...
def region_2d_to_location_3d(region: 'bpy.types.Region',
    ...
    :param depth_location: the returned vectors depth is aligned with this
    since there is no defined depth with a 2d region input.
...
def region_2d_to_origin_3d(region: 'bpy.types.Region',
    ...
    :param coord: 2d coordinates relative to the region; (
    event.mouse_region_x, event.mouse_region_y) for example.
    :param clamp: Clamp the maximum far-clip value used. (negative value will
    move the offset away from the view_location)
